...
**Version**: 3.10.12 | **Date**: July 2026
...
---
*Generated automatically by Wire Framework v3.10.12 · `/wire:dbt-migration-generate 01-gdp-snowflake-to-bq`*
````
...
- **`/wire:dbt-migration-pre-pr-review`** runs a faithfulness review over the translated diff before a PR is opened, composing the checks above plus the pair's edge-case runtime patterns (an uncast blank-string-to-numeric, an unguarded JSON parse, an unanchored regex) into a structured findings list with severity, `file:line`, and a suggested fix. Run it with `--format json --severity error` to gate CI, so the defects are resolved locally instead of in the client's PR queue.
...
### v3.10.12 — Batch-zero macro & UDF pass, single-SCC batching fallback (July 2026)
