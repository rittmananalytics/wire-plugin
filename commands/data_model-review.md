---
description: Review data model with team
...
---
description: Record stakeholder review feedback on data model
argument-hint: <project-folder>
---
...
Record stakeholder feedback on the data model. Captures approval or change requests.

## Usage

```bash
/wire:data_model-review YYYYMMDD_project_name
```

## Prerequisites

- data model must exist and pass validation
- `data_model.validate` should be `pass`
...
**Process**:
1. Read `status.md`
2. Check that `data_model.validate == pass`

**If validation not pass**:
```
Warning: data model has not passed validation yet.

Run `/wire:data_model-validate <project>` before review.
...
Please review the data model and provide feedback.
```
...
1. Follow the meeting context retrieval workflow defined in `specs/utils/meeting_context.md`
   - Pass the project folder and artifact name `data_model`
...
```json
{
  "questions": [{
    ...
    "options": [
      {"label": "Approved", "description": "data model is complete and approved"},
      {"label": "Changes requested", "description": "data model needs revisions"},
...
**Ask for reviewer**:
```
Who approved the data model? (Name and role)
```
...
1. Address feedback
2. Re-validate: `/wire:data_model-validate <project>`
