...
---
description: Record stakeholder review feedback on data quality
argument-hint: <project-folder>
---

# data quality Review Command

## Purpose

Record stakeholder feedback on the data quality. Captures approval or change requests.

## Usage

```bash
/wire:data_quality-review YYYYMMDD_project_name
```

## Prerequisites

- data quality must exist and pass validation
- `data_quality.validate` should be `pass`
...
**Process**:
1. Read `status.md`
2. Check that `data_quality.validate == pass`

**If validation not pass**:
```
Warning: data quality has not passed validation yet.

Run `/wire:data_quality-validate <project>` before review.
...
Please review the data quality and provide feedback.
```
...
1. Follow the meeting context retrieval workflow defined in `specs/utils/meeting_context.md`
   - Pass the project folder and artifact name `data_quality`
...
```json
{
  "questions": [{
    ...
    "options": [
      {"label": "Approved", "description": "data quality is complete and approved"},
      {"label": "Changes requested", "description": "data quality needs revisions"},
...
**Ask for reviewer**:
```
Who approved the data quality? (Name and role)
```
...
**Suggest iteration**:
```
## data quality Changes Requested 🔄
...
1. Address feedback
2. Re-validate: `/wire:data_quality-validate <project>`
