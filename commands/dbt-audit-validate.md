...
**Check 1 — Catalogue reconciles with disk**
Independently walk every resolved project's filesystem (per `specs/utils/dbt_manifest_parse.md` Steps 1 and 3) and compare against the CSV. This runs first, before any count- or completeness-based check, because those checks are meaningless against a stale or substituted catalogue — this is the backstop that catches that regardless of how it went stale, and it must not rely on generate's own report of what it did.
...
**Check 4 — Batch ordering respects dependency graph**
...
**Check 5 — Feature tags applied to all models**
...
**Check 8 — Macros with adapter functions flagged**
Every model whose transitive macro-usage set (from the Check 4 re-parse) intersects the NEEDS-translation set has that intersection recorded, comma-separated, in `platform_macros`. Every macro classified as NEEDS-translation carries an `action` of `translate`, `redesign`, or `manual-review-out-of-scope` — none left unclassified.
