...
---
description: Record stakeholder review feedback on deployment
argument-hint: <project-folder>
---

# deployment Review Command

## Purpose

Record stakeholder feedback on the deployment. Captures approval or change requests.

## Usage

```bash
/wire:deployment-review YYYYMMDD_project_name
```

## Prerequisites

- deployment must exist and pass validation
- `deployment.validate` should be `pass`
...
**Process**:
1. Read `status.md`
2. Check that `deployment.validate == pass`

**If validation not pass**:
```
Warning: deployment has not passed validation yet.

Run `/wire:deployment-validate <project>` before review.
...
Please review the deployment and provide feedback.
```
...
1. Follow the meeting context retrieval workflow defined in `specs/utils/meeting_context.md`
   - Pass the project folder and artifact name `deployment`
...
```json
{
  "questions": [{
    ...
    "options": [
      {"label": "Approved", "description": "deployment is complete and approved"},
      {"label": "Changes requested", "description": "deployment needs revisions"},
...
**Ask for reviewer**:
```
Who approved the deployment? (Name and role)
```
...
1. Address feedback
2. Re-validate: `/wire:deployment-validate <project>`
