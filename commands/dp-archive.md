...
Move a completed project to the archive folder to keep the active project list small. Archived projects are excluded from `/wire:dp-status` and `/wire:dp-start` scans but remain accessible via `/wire:dp-status --archived`.

## Usage

```bash
/wire:dp-archive 20260210_rowcal
```
...
{
  "questions": [{
    "question": "Archive this project? It will be moved to .wire/archive/ and hidden from /wire:dp-status and /wire:dp-start.",
    "header": "Confirm",
    "options": [
...
The project won't appear in `/wire:dp-status` or `/wire:dp-start`.

To view archived projects: `/wire:dp-status --archived`
```
