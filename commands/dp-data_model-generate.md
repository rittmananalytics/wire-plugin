...
```bash
/wire:dp-data_model-generate YYYYMMDD_project_name
```
...
4. **Otherwise (default)**:
   - Verify all three default prerequisites are met. For each that is not:
     ...
     Run: /wire:dp-[artifact]:review <project_id>
     ```
   - Read the following in order:
     - `requirements/requirements_specification.md`
...
1. Validate the data model:
   /wire:dp-data_model-validate <project_id>

2. After validation, review with analytics engineering lead:
   /wire:dp-data_model-review <project_id>
...
These DDL files serve as inputs for seed data generation (`/wire:dp-seed_data-generate`) and the dbt project structure.
