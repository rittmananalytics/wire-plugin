...
```bash
/wire:dp-dbt-validate YYYYMMDD_project_name
```

## Prerequisites

- dbt models must be generated (`/wire:dp-dbt-generate` complete)
- dbt models must be run successfully (`/wire:dp-utils-run-dbt` complete)
...
Run `/wire:dp-dbt-generate [folder]` first.
```
...
2. **Review dbt models with team**: `/wire:dp-dbt-review [folder]`
3. **Generate semantic layer**: `/wire:dp-semantic_layer-generate [folder]`
```
...
Run dbt models: /wire:dp-utils-run-dbt [folder]
```
