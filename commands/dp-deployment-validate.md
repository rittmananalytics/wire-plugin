...
```bash
/wire:dp-deployment-validate YYYYMMDD_project_name
```

## Prerequisites

- deployment must be generated (`/wire:dp-deployment-generate` complete)
...
Run `/wire:dp-deployment-generate <project>` first.
```
...
1. **Review with stakeholders**: `/wire:dp-deployment-review <project>`
```
