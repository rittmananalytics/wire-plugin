...
```bash
/dp:deployment:validate YYYYMMDD_project_name
```

## Prerequisites

- deployment must be generated (`/dp:deployment:generate` complete)
...
Run `/dp:deployment:generate <project>` first.
```
...
1. **Review with stakeholders**: `/dp:deployment:review <project>`
```
