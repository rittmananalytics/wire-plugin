...
2. **Generate requirements** from the SOW:
   /wire:dp-requirements-generate {folder_name}

3. **Check project status** anytime:
   /wire:dp-status {folder_name}

4. **When ready to merge**, create a pull request:
   /wire:dp-utils-create-pr {folder_name}

### Quick Commands for This Project

| Command | Purpose |
|---------|---------|
| `/wire:dp-status {folder_name}` | View project status and next actions |
| `/wire:dp-requirements-generate {folder_name}` | Extract requirements from SOW |
| `/wire:dp-workshops-generate {folder_name}` | Generate workshop materials |
| `/wire:dp-pipeline_design-generate {folder_name}` | Design data pipeline |
| `/wire:dp-dbt-generate {folder_name}` | Generate dbt models |
| `/wire:dp-training-generate {folder_name}` | Generate training materials |
```
