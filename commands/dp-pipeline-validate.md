...
```bash
/dp:pipeline:validate YYYYMMDD_project_name
```

## Prerequisites

- pipeline must be generated (`/dp:pipeline:generate` complete)
...
Run `/dp:pipeline:generate <project>` first.
```
...
1. **Review with stakeholders**: `/dp:pipeline:review <project>`
```
