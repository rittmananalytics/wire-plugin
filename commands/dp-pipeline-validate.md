...
```bash
/wire:dp-pipeline-validate YYYYMMDD_project_name
```

## Prerequisites

- pipeline must be generated (`/wire:dp-pipeline-generate` complete)
...
Run `/wire:dp-pipeline-generate <project>` first.
```
...
1. **Review with stakeholders**: `/wire:dp-pipeline-review <project>`
```
