...
```bash
/dp:training:validate YYYYMMDD_project_name
```

## Prerequisites

- training must be generated (`/dp:training:generate` complete)
...
Run `/dp:training:generate <project>` first.
```
...
1. **Review with stakeholders**: `/dp:training:review <project>`
```
