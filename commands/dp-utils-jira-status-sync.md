...
Perform a full reconciliation of all artifact lifecycle states between local `status.md` and Jira. Syncs every in-scope artifact's Sub-tasks, Tasks, and Epic in a single pass. Called by `/wire:dp-status` to ensure Jira stays in sync.

## Usage

```bash
/wire:dp-utils-jira-status-sync YYYYMMDD_project_name
```

Typically invoked automatically by `/wire:dp-status` when Jira is configured.
...
- Never block the `/wire:dp-status` command

**Issue keys missing from status.md:**
- Skip artifacts without Jira keys
- Report: `Note: [artifact] has no Jira keys — run /wire:dp-utils-jira-create to set up tracking`
