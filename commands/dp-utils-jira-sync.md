...
```bash
/wire:dp-utils-jira-sync YYYYMMDD_project_name requirements generate
```
...
- Project must have Jira keys in `status.md` (created by `/wire:dp-utils-jira-create`)
...
**Next**: Validate → /wire:dp-[artifact]:validate [folder]
```
...
**Next**: Submit for review → /wire:dp-[artifact]:review [folder]
```
