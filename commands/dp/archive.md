...
Move a completed project to the archive folder to keep the active project list small. Archived projects are excluded from `/dp:status` and `/dp:start` scans but remain accessible via `/dp:status --archived`.

## Usage

```bash
/dp:archive 20260210_rowcal
```
...
To view archived projects: `/dp:status --archived`
```
