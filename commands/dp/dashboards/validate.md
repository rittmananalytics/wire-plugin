...
```bash
/dp:dashboards:validate YYYYMMDD_project_name
```

## Prerequisites

- dashuoards must be generated (`/dp:dashboards:generate` complete)
...
Run `/dp:dashboards:generate <project>` first.
```
...
1. **Review with stakeholders**: `/dp:dashboards:review <project>`
```
