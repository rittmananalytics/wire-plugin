...
```bash
/dp:data_quality:validate YYYYMMDD_project_name
```

## Prerequisites

- data quality must be generated (`/dp:data_quality:generate` complete)
...
Run `/dp:data_quality:generate <project>` first.
```
...
1. **Review with stakeholders**: `/dp:data_quality:review <project>`
```
