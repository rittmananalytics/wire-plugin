...
```bash
/dp:dbt:validate YYYYMMDD_project_name
```

## Prerequisites

- dbt models must be generated (`/dp:dbt:generate` complete)
- dbt models must be run successfully (`/dp:utils:run-dbt` complete)
...
Run `/dp:dbt:generate [folder]` first.
```
...
2. **Review dbt models with team**: `/dp:dbt:review [folder]`
3. **Generate semantic layer**: `/dp:semantic_layer:generate [folder]`
```
...
Run dbt models: /dp:utils:run-dbt [folder]
```
