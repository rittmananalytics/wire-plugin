...
2. **Generate requirements** from the SOW:
   /dp:requirements:generate {folder_name}

3. **Check project status** anytime:
   /dp:status {folder_name}

4. **When ready to merge**, create a pull request:
   /dp:utils:create-pr {folder_name}

### Quick Commands for This Project

| Command | Purpose |
|---------|---------|
| `/dp:status {folder_name}` | View project status and next actions |
| `/dp:requirements:generate {folder_name}` | Extract requirements from SOW |
| `/dp:workshops:generate {folder_name}` | Generate workshop materials |
| `/dp:pipeline_design:generate {folder_name}` | Design data pipeline |
| `/dp:dbt:generate {folder_name}` | Generate dbt models |
| `/dp:training:generate {folder_name}` | Generate training materials |
```
