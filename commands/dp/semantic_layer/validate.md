...
```bash
/dp:semantic_layer:validate YYYYMMDD_project_name
```

## Prerequisites

- Semantic layer must be generated (`/dp:semantic_layer:generate` complete)
...
Run `/dp:semantic_layer:generate <project>` first.
```
...
2. **Review with stakeholders**: `/dp:semantic_layer:review <project>`
