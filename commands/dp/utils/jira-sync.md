...
```bash
/dp:utils:jira-sync YYYYMMDD_project_name requirements generate
```
...
- Project must have Jira keys in `status.md` (created by `/dp:utils:jira-create`)
...
**Next**: Validate → /dp:[artifact]:validate [folder]
```
...
**Next**: Submit for review → /dp:[artifact]:review [folder]
```
