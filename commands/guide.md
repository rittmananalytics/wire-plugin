---
description: Interactive co-pilot — orients new users and surfaces the right next action for navigating consultants
argument-hint: [new/resume/explain]
---

# Interactive co-pilot — orients new users and surfaces the right next action for navigating consultants
...
---
description: Interactive Wire co-pilot — orients new users and surfaces the right next action for consultants who need direction
argument-hint: [new|resume|explain]
---
...
`/wire:guide` is the Wire co-pilot. It serves two audiences:
...
2. **Navigating users** — experienced consultants who know Wire but aren't sure what to run next, don't understand why certain steps exist, or have lost track of where they are in a project
...
Based on the results, output one of the following blocks — always as the very first output of the guide, before any other content.
...
After Step 3, re-run /wire:guide and continue from here.
...
completely, reopen it, and run /wire:guide again.
...
| Wire never used in this repo but Wire is installed | New-user onboarding (Alex-type: aware of Wire, hasn't started) |

**Lightweight session-start mode** (applies in navigational mode): If the user has run `/wire:guide` in this repo within the past 48 hours and no new changes have been made to `.wire/` since then, skip the intent questions and go directly to the Phase 4 output block with current state summary and top next action. Users switching between multiple repos multiple times per day need fast reorientation, not a full interactive session.
...
The framework's most important commands for day-to-day work:
  ...
  /wire:guide           — this command — surfaces what to do next
...
Before handing off to `/wire:new`, explain the cycle once — because Sav- and Lydia-type issues (skipping review, thinking validate is redundant) stem from never understanding the purpose of each step:
...
Used when Wire is already set up in the repo and the user needs to know what to do next.
...
Output the current state summary, then ask:
...
Map the user's intent (or free-text response) to a specific recommended action using this table:
...
NAVIGATION
  /wire:guide                   This command — what do I do next?
...
```markdown
## Wire Guide — [Mode: Onboarding / Navigation / Explanation]
