...
- **`dbt-migration-pre-pr-review`** — a faithfulness review over the translated diff, run before a PR is opened. It composes the three checks above plus the pair's edge-case runtime patterns (an uncast blank-string-to-numeric, an unguarded JSON accessor, an unanchored regex) into a structured findings list with severity, `file:line`, and a suggested fix. Run it with `--format json --severity error` to gate CI, so defects are resolved locally instead of in the client's PR queue.
