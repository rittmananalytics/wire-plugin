...
The audit and migration commands reach beyond Wire's standard integrations — they connect directly to your source platform, target platform, ingestion service, and orchestration layer to read schema, inspect pipeline state, and run equivalency checks. Configure the relevant connections in `.claude/settings.json` before running the first audit command. See [MCP Servers](../reference/mcp-servers) for the full list of supported servers and connection setup.
