...
| `verdicts[].verdict` | A taxonomy value: `pass` \| `pass_qualified` \| `diff_vintage` \| `diff_availability` \| `diff_schema_type` \| `fail` (see `equivalency-validate`, Verdict taxonomy). |
...
5. **`post_merge_prod` rows touch delivery, not equivalence.** A `post_merge_prod` verdict never updates the `last_equivalence_*` columns. If the verdict is `pass` or `pass_qualified` **and** the model's `delivery_stage` is `merged`, set `delivery_stage: production_verified`. If `delivery_stage` is not `merged`, leave the register untouched and report `not_merged` — a production verdict for something not on the client's main is evidence filed for later, not a stage advance.
